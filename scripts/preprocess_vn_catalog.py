...
def normalize_catalog_fields(json_path: Path) -> int:
...
def build_chroma(json_path: Path, chroma_dir: Path) -> int:
    ...
    collection = client.get_or_create_collection("lux_quartz_vn_products")
    ...
    for product in products:
        ...
        variant_prices = [
            v.get("price_slab_vnd")
            for v in product.get("variants", [])
            if v.get("price_slab_vnd") is not None
        ]
        if not variant_prices:
            variant_prices = [
                v.get("price_sqm_vnd")
                for v in product.get("variants", [])
                if v.get("price_sqm_vnd") is not None
            ]
        price_min = min(variant_prices) if variant_prices else None

        description = product.get("description") or ""
        chunk = (
            f"Mã {code}, màu {product.get('color')}, nhóm màu {product.get('color_group')}, "
            f"tên dòng {product.get('series')}, hoa văn {product.get('pattern')}. {description}"
        ).strip()
        ...
        metas.append(
            {
                ...
                "market": "VN",
                "source": product.get("description_source") or "VN",
            }
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Preprocess Lux Quartz VN catalog")
    parser.add_argument(
        "--input",
        default="c:/My Web Sites/clonelandingpage/Lux Quartz VN.csv",
        help="Input CSV path",
    )
    parser.add_argument(
        "--json-output",
        default="c:/My Web Sites/clonelandingpage/data/lux_quartz_vn_products.json",
        help="Output JSON path",
    )
    parser.add_argument(
        "--chroma-dir",
        default="c:/My Web Sites/clonelandingpage/data/chroma_vn",
    ...
    catalog = load_catalog(input_path, market="VN")
    save_catalog_json(catalog, json_path)
    normalized_count = normalize_catalog_fields(json_path)
